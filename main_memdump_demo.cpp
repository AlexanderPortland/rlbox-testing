...
//     sandbox.reset_sandbox();
    ...
    printf("main: secret_num--%d, %p\n", *secret_num_ptr, secret_num_ptr);
    printf("main: const_secret_num--%d, %p\n", *const_secret_num_ptr, const_secret_num_ptr);

    auto a_res = sandbox.invoke_sandbox_function(a);

    long *k_ptr = a_res.UNSAFE_unverified();

    printf("main: k--%ld at %p\n", *k_ptr, k_ptr);
